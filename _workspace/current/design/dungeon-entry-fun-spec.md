...
## 18. W5 「명령 각인」 — 텍스트 입력을 저작 수단으로

작성 2026-08-12. 사용자 요구: *"핵심재미요소에 텍스트 입력을 통한 요소도
추가"*. **상태: 설계 초안 — 미승인. 경제 확인 1건·폰트 작업 1건 선행.**

### 18.1 왜 이것이 선택 압력의 구조적 해답인가

§17이 남긴 부채 1번은 **초반 선택 압력이 구조적으로 최저**(0.400)이고,
"유일한 해법은 초반 기믹 종류 증가인데 G2 무변경과 충돌"이라고 적었다.
텍스트 입력은 **제3의 해법**이다 — 풀을 늘리지 않고 제시 수를 줄인다.

`선택압력 = 1 − (제시 수 / 풀 크기)` (§4.2)

| 경로 | 제시 | 풀 | 압력 |
|---|---|---|---|
| 현재 (B6·B7) | 3 | 3 | **0.00** |
| W3 현장 각인 | 3 | 5–11 (해저드 필터 후) | 0.400–0.727 |
| **W5 타이핑** | **1** (내가 부른 것) | 같은 풀 | **0.80–0.91** |

**타이핑은 제시 목록에서 고르는 행위가 아니라 풀 전체에서 호출하는 행위다.**
제시 수가 1로 떨어지므로 압력이 풀 크기와 무관하게 밴드 위로 올라간다.
§4.4 해저드 필터가 만든 풀 크기 상한이 더는 압력의 천장이 아니다.

이것은 §4.2가 인용한 Hades 선례의 반대 방향이다. Hades는 **선택지를 줄이는
것을 난이도로 판다**(Approval Process, 랭크당 −1). W5는 선택지를 줄이지
않고 **제시 단계를 건너뛴다** — 값은 시간으로 지불한다(§18.4).

### 18.2 기반은 이미 있다 `[OBSERVED — 코드 실측]`

**신규 입력 시스템이 아니다.** 텍스트 입력 경로가 전부 구현돼 있다:

| 부품 | 위치 | 하는 일 |
|---|---|---|
| 커맨드 콘솔 | `HudView.cs:2071-2100` | 필드·토스트·문자 상한 60 |
| 한글 IME 버퍼 | `CommandConsoleBuffer.cs` | 같은 프레임 중복 음절 제거 |
| 조합 상태기 | `CommandConsoleImeComposition.cs` | 미확정(pre-edit) 처리 |
| WebGL IME 브리지 | `WebGLHangulIme.cs` | emscripten이 canvas에 조합 이벤트를 주지 않는 문제 우회 — 숨은 `<input>`이 키보드를 받는다 |
| 키워드 파서 | `CommandPlanParser.ParseLocal` (`HudView.cs:2363`) | 위치 순서대로 다단 계획 |
| 입력 게이트 | `InputAdapter.TextInputActive` (`:137,:141`) | 타이핑 중 조작 키 차단 |

현재 용도는 **수호자 명령**(집중공격/방어/복귀 + 스킬 시전)이다. W5는 이
표면을 **레벨업 오퍼로 확장**하는 것이고, 두 번째 입력 경로를 만들지 않는다.

### 18.3 형태

레벨업 오퍼(`GrowthOfferOpen`)가 열린 동안 콘솔을 열어 **각인 이름을
타이핑**하면, 제시된 3개가 아니라 그 스테이지 풀 전체에서 그 면을 가져온다.

```
레벨업 → 오퍼 3장 제시 (기존, 1/2/3 키)
       → [선택] 콘솔 열고 각인 이름 타이핑 → 풀 전체에서 호출
```

- **1/2/3 키 경로는 완전히 불변.** 타이핑을 모르는 플레이어는 지금과 동일하게
  플레이한다(§1 설계 원칙 3 "각 뎁스 층은 몰라도 플레이가 되어야 한다",
  `input-depth-spec.md:23`).
- 어휘는 **고정 사전**이다 — 자유 문장이 아니다(§18.5 폰트 제약).
- 오퍼 창은 5.0초(`HackSpec.GrowthOfferSeconds`)이고 **심은 그 동안 멈추지
  않는다**(`CinderSim.cs:1743` `_growthOfferTime -= deltaTime`). 이것이 가격이다.

### 18.4 가격 — 시간으로 지불한다

W5는 "항상 최선의 선택지를 얻는" 힘이므로 값이 매겨져야 한다.
**값은 실시간 전투 위험이다:**

| 지불 항목 | 실측 근거 |
|---|---|
| 타이핑하는 동안 조작 불가 | `InputAdapter.TextInputActive`가 이동·공격 키를 차단 |
| 심은 계속 진행 | 오퍼 창은 일시정지가 아니다 (`:1743`) |
| 실패 시 자동 확정 | 5초 만료 시 `GrowthChoiceKind.None` (`:1746`) — **아무것도 못 얻는다** |

웨이브 한가운데서 3초를 손에서 놓는 것은 실제 위험이다. 안전할 때만
타이핑할 수 있으므로 **"언제 저작할 수 있는가"가 새 판단 축이 된다.**

**PM 확인 필요 1건**: 이 시간 비용이 entry 18(현장 각인 크기 ≤50%) 가격을
대체하기에 충분한가. 효과 크기는 W3와 동일하고 신규 자원·신규 보상은 0이므로
**경제 축 신설은 없지만**, 선택 품질 상승분은 미가격이다. 서명 대상.

### 18.5 폰트 제약 — 자유 문장은 불가능하다 `[OBSERVED]`

`tools/gen_hud_font.sh:26-44`가 `Assets/Scripts/View/*.cs`의 **문자열
리터럴에서만** 글리프를 수확해 NanumBarunGothic을 그 집합으로 서브셋한다.
HUD 폰트는 `ViewTypography.ResolveFont()` → `Resources.Load<Font>
("Fonts/HudKorean")`로 그 서브셋을 읽는다(`ViewTypography.cs:22`).

→ **소스에 없는 음절은 두부(tofu)로 렌더된다.** 한글 음절은 11,172자이고
서브셋은 그 일부다. 따라서 W5 어휘는 **View 소스에 리터럴로 존재하는 고정
사전**이어야 한다.

이미 존재하는 어휘 `[OBSERVED]`:

```
LobbyView.cs:75  SigilNames    = { "역류인", "판결인", "집행인", "점화인", "증언인" }
LobbyView.cs:78  SigilGimmicks = { "해류", "방벽주", "장벽", "분출구", "제단" }
```

각인 5종의 이름은 전부 이미 아틀라스에 있다. **A/B 면 이름은 없다** —
면을 지정하려면 `sigil-spec.md` §각인 표의 면 이름(역류 저항/와류/관통
판결/파쇄/집행 저항/처형/재점화/연쇄/속기/증폭)을 View 리터럴로 추가하고
`tools/gen_hud_font.sh`를 재실행해 폰트를 동반 커밋해야 한다(§4b).

**선행 작업 1건**: 위 10개 면 이름 리터럴 추가 + 폰트 재생성 +
`getBestCmap()` 검사(합집합 프로브 금지 — §10.1과 동일 프로토콜).

### 18.6 결정론 — 원문은 심에 들어가지 않는다

| 층 | 다루는 것 |
|---|---|
| View | 타이핑 원문 → 고정 사전 대조 → 정수 코드 |
| 심 | **정수만** 받는다 |

`SimInput.GrowthChoice`가 이미 정수 래치다(`InputAdapter.cs:98`
`QueueGrowthChoice(int)`, `CinderSim.cs:1737-1742` `picked >= 1 && <= 3`).
W5는 그 **범위를 확장**한다 — 신규 필드 0개, 신규 타입 0개.

- 원문 문자열은 심 경계를 넘지 않는다 → 무RNG·결정론 무영향.
- 사전 대조는 View의 순수 함수 → 같은 입력에 같은 코드.
- 미옵트인 경로에서 확장 코드는 도달 불가 → **골든 무이동**(§9 W3와 동일 구조).

### 18.7 텔레메트리 — 신규 필드가 필요하다 (§1c 판정의 예외 확장)

`ops/telemetry-contract.md`의 cycle-9 판정은 "신규 필드 0개 — 결정론이
텔레메트리를 대체한다"였고, **예외 2개**를 이미 인정했다: G8 인상 점수와
오퍼 **수용** 여부(플레이어 입력이라 재현에 입력 로그가 필요).

W5는 그 두 번째 예외와 **같은 종류**다 — 타이핑 여부·타이핑 소요 시간은
플레이어 입력이므로 재현으로 대체할 수 없다. 세션 관찰로 대체 가능한지
판정해야 한다. **미판정 항목으로 등재한다.**

### 18.8 검증 계약 (초안)

| # | 항목 | 방법 |
|---|---|---|
| W5-1 | 1/2/3 경로 무변경 | 미옵트인 골든 바이트 동일 |
| W5-2 | 사전 대조 결정론 | 동일 원문 2회 → 동일 정수 |
| W5-3 | 사전 외 입력 거부 | 오타·미등록어 → 코드 0, 오퍼 유지 |
| W5-4 | 폰트 커버리지 | 사전 전 어휘를 `getBestCmap()` 대조, MISSING 0 |
| W5-5 | 선택 압력 | 타이핑 경로에서 제시=1 실측, 9스테이지 전수 |
| W5-6 | 시간 비용 실재 | 타이핑 중 이동·공격 입력이 심에 도달하지 않음 |
| W5-7 | 만료 안전 | 5초 초과 시 `None` 확정, 상태 오염 0 |
| W5-8 | IME 왕복 | WebGL 실브라우저에서 조합→확정→제출, 중복 음절 0 |

### 18.9 이 절이 하지 않는 것

| 안 함 | 이유 |
|---|---|
| 자유 문장 명령 | 폰트 서브셋이 렌더 불가(§18.5). 기존 콘솔의 자유 문장 광고는 별건 결함으로 등재 — `qa/defect-register.md` C9-DEF-005 |
| 원격(Gemini) 해석 | 런타임 키 의존은 결정론 밖. W5는 로컬 사전만 |
| 오퍼 창 일시정지 | 정지하면 가격이 사라진다(§18.4) |
| 신규 각인 종류 | 기존 5종 × A/B만. §4.4 필터 재설계는 여전히 cycle-10 |
| 모바일 타이핑 | §18.10 — 물리 키보드 없는 플랫폼은 기존 탭 경로가 정식 경로다 |

### 18.10 플랫폼 판정 — 모바일은 폴백이 아니라 정식 경로다 `[확정 필요]`

375×667은 QA 세 뷰포트 전부의 게이트 대상이다(`qa/stage-hazard-remaster/
final-512/report.md:13-15` desktop-1920 / desktop-1280 / mobile-375, 각
36체크포인트). 따라서 모바일을 "미판정"으로 남기면 구현 후 반송된다.

**모바일에서 타이핑이 성립하지 않는 이유** `[측정 필요 — 산술 근거]`:

| 항목 | 값 |
|---|---|
| 오퍼 창 | 5.0초, 심 미정지 (`HackSpec.GrowthOfferSeconds`, `CinderSim.cs:1743`) |
| 각인 3음절 입력 | 자모 약 9타 + 후보 확정 |
| OS IME 패널 | 667px 화면의 아래 절반을 덮는다 — **가격 대상인 전투가 가려진다** |

타이핑 비용이 "전투 위험"인데(§18.4) 그 전투가 키보드에 가려지면 가격이
성립하지 않는다. 위험을 지불하는 것이 아니라 눈을 감는 것이 된다.

**판정: 축을 플랫폼별로 나눈다.**

| 플랫폼 | 저작 경로 | 압력 |
|---|---|---|
| 데스크톱 (물리 키보드) | 타이핑 = 풀 전체 호출 + 기존 1/2/3 | 0.80–0.91 |
| 모바일 (터치) | **기존 온스크린 탭 1..3** — `InputAdapter.cs:97-98`이 이미 "on-screen level-up choice tap (1..3)"로 존재 | 0.400–0.727 (W3 값) |

모바일은 **기능이 빠진 것이 아니라 W3까지를 받는다.** W5는 W3 위의 추가
층이고, §1 설계 원칙 3("각 뎁스 층은 몰라도 플레이가 되어야 한다")이 정확히
이 경우를 위해 있다.

**등재하는 비대칭**: 데스크톱이 모바일보다 선택 압력이 높다 → 같은 진행
밴드로 두 플랫폼을 채점할 수 없다. G7 판정은 **플랫폼별로 분리**해야 하고,
§4.2 진행 밴드(0.70/0.80/0.85)는 현재 모바일에서 달성 불가다. 이것은 W5가
만든 문제가 아니라 §17 부채 1번(초반 압력 구조적 최저)이 플랫폼 축에서
다시 드러난 것이다.

**대안 (미채택, cycle-10 검토)**: 모바일 전용으로 오퍼 창을 늘리는 대신
**풀 전체를 스크롤 목록으로 제시**하면 제시 수가 풀 크기와 같아져 압력이
0으로 돌아간다 — 타이핑의 이점이 사라지므로 기각. 각인 이름 **앞 1음절
탭 선택**(5종 = 5버튼)이 더 유망하나, 그것은 타이핑이 아니라 새 UI이고
예고 예산·터치 플로어 44px 재센서스가 붙는다.
