...
### 2.2b 난이도 UI — 후속으로 처리한 것 (View 전용)

§16 은 티어를 **런 생성 시점에 고정**한다(2.3-3). 고정된 선택은 보이지 않으면
함정이 된다 — 로비에서 악몽을 고르고 죽었을 때, 어떤 규칙이 자신을 죽였는지
화면 어디에도 없었다. 티어를 말하는 곳은 로비 버튼 하나뿐이고 그때쯤이면 세 화면
전이다. 감사 결과 인런 HUD·결과 화면 어디에도 티어 표기가 **없었다**.

- **인런 난이도 배지** (`View/HudView.cs`): 스탯 패널(웨이브/점수/유물/적)에
  5번째 행을 추가하고 `GameView.Begin` 에서 `SetRunDifficulty` 로 런당 1회
  래치한다(`SetTrialMode` 와 같은 규율 — 심은 런 중 티어를 바꿀 수 없으므로
  매 프레임 폴링은 값의 성질을 왜곡한다). 표기는 `난이도 악몽 · 협동` 한 줄.
- **기준선에서는 아무것도 안 보인다**: `Difficulty.Normal` 은 개정 전 규칙
  그대로이고 아레나·프롤로그가 항상 쓰는 값이라, 거기서 `난이도 보통` 행은
  영구적 노이즈다. 배지는 **이탈 표시**다 — 규칙이 기본값이 아닐 때만 뜬다.
  따라서 패널 높이도 108↔132 로 같이 움직이고(빈 배경 금지), 그 아래
  음소거 버튼이 8 u 간격을 유지한 채 따라 내려간다.
- **결과 라인에 티어 동반**: 게임오버 `_finalText` 와 구역 정화
  `_stageClearText` 에 `DifficultyResultSuffix` 를 붙였다. 입문에서 딴 점수와
  악몽에서 딴 점수는 같은 수가 아니다. Normal 은 빈 문자열을 반환하므로
  기존 결과 라인은 바이트 단위로 동일하다.
- **로비 사이클 버튼에 단계 표시** (`View/LobbyView.cs`): `난이도: 어려움 [3/4]`.
  사이클 버튼은 한 번에 한 티어만 보여주므로, 표시가 없으면 몇 개가 있는지도
  클릭이 어느 방향인지도 알 수 없다. 순서는 `DifficultySpec.OrderOf`(쉬운 것부터)
  라서 숫자 자체가 진행 방향을 말한다.
...
- `[OBSERVED]` **난이도 UI(§2.2b) 후속 검증**: `DifficultyHudTests.cs` 11건 신규,
  전체 스위트 **582/582 통과**(실패 0 / 스킵 0) —
  `/tmp/hongT-ui/_workspace/current/engineering/unity-logs/test-results-184222.xml`.
  (`/tmp/hongT-build` 는 실행 도중 다른 세션의 WebGL 빌드가 점유·덮어써서 결과가
  571건으로 바뀌었다. 그래서 격리 사본 `/tmp/hongT-ui` 를 새로 떠서 재실행했다.)

  같은 실행에서 `HudLayoutTests` 19건(폰 티어 겹침·터치 하한 계약)과
  `FontCoverageTests` 가 통과해 패널 높이 변경이 레이아웃 계약을 깨지 않았음이,
  `DungeonGoldenDigestTests` 7건 통과로 심이 손대지지 않았음이 확인된다.
  배지 문자열은 기존 어휘(난이도/입문/보통/어려움/악몽/협동)만 쓰므로 서브셋
  폰트 재생성이 필요 없다.
