...
2. **장비 파밍 가짓수 확장** — `[INFERENCE]`
   - 01:41 / 04:28 / 06:28 의 "무기와 갑옷·아뮬렛이 전부라 파밍 재미가 떨어진다"
     지적은 **우리에게 그대로 적용된다**. 현재 축은 3슬롯(`weapon` / `lantern` /
     `cloak`, `Sim/CampaignTypes.cs:24 EquipSlot`) 선형 강화 T0–T5 + 각인 2슬롯이라
     슬롯 수가 리뷰 대상작과 동급이다. 메타 루프 콘텐츠 영역이므로 별도 개정으로 다룬다.
...
4. **무기 계열별 이질적 전투 (검방패 / 양손검 / 도끼 / 창)** — `[INFERENCE]`
   - 05:47–05:58 은 무기 계열마다 공격 방식·속성·전용 스킬이 다른 점을 장점으로
     든다. 우리 `weapon` 슬롯은 `WeaponDamagePerRank = 0.06f` 단일 스칼라 축이며
     계열 개념이 없다. 계열 추가는 액션 라이브러리 11종을 계열 수만큼 복제해야
     하므로 애셋 파이프라인 규모의 작업이다 — 이번 개정 범위 밖.

5. **장비 등급 + 랜덤 드랍 (일반/희귀/전설/신화)** — `[INFERENCE]`
   - 06:03 의 4등급 체계는 랜덤 롤 파밍을 전제한다. 우리 강화는 결정론적 선형
     T0–T5 이고, 확률 롤은 골든 다이제스트 재현성과 직접 충돌한다. 도입하려면
     드랍 RNG 를 심 시드에 편입하는 별도 설계가 선행돼야 한다.

6. **보스전 소울라이크 방어 컨트롤 (패링 / 막기)** — `[INFERENCE]`
   - 04:40 은 보스전에서 회피·막기·패링을 요구한다고 말한다. 우리 방어 수단은
     대시 i-frame(`DashDistance 190` / `DashTime 0.22`)과 Void Aegis 캐스트 무적
     뿐이다. `ActorAction.Defence` 는 **Aegis 캐스트 포즈**이지(`CinderSim.cs:1312`)
     지속 가드가 아니다. 패링은 타이밍 창·성공 판정·반격 분기를 새로 여는 일이라
     이번 범위 밖.

7. **사망 페널티 (경험치 드랍 + 시체 회수)** — `[INFERENCE]`
   - 05:02–05:09. 우리는 로그라이트 런 구조라 런 종료가 곧 정산이며, 월드에
     시체를 남기는 회수 루프는 구조적으로 맞지 않는다. **의도적 비채택**.

8. **거대 스킬트리** — `[INFERENCE]`
   - 05:11–05:45 의 힘/지구력/활력/정확도 + 특수 스킬 트리 대비, 우리는 3스탯
     (공격/체력/이속) + 각인 2슬롯으로 훨씬 얕다. 깊이 확장은 2번과 같은
     메타 루프 개정 묶음이다.

9. **NG+ / 2회차 전용 모드** — `[INFERENCE]`
   - 06:38–06:49 의 캐릭터·템 유지 재시작, 07:14–07:27 의 "2회차용 새 모드가
     있으면 좋겠다"는 리뷰어 본인의 미충족 희망사항이다. 우리 메타 진행은 런을
     가로질러 유지되므로 부분적으로 대응되지만, 전용 2회차 모드는 없다.
...
- `[OBSERVED]` **EditMode 전체 스위트 527/527 통과**(실패 0 / 스킵 0). 이 세션의
  최초 시도는 다른 세션의 Unity 에디터(PID 16568)가 프로젝트를 점유해
  "another Unity instance is running" 으로 거부당했으므로, 커밋된 소스를
  `/tmp/hongT-build` 사본으로 rsync 해 그쪽에서 배치모드를 돌렸다. 결과:
  `/tmp/hongT-build/_workspace/current/engineering/unity-logs/test-results-170750.xml`.
  신규 25건(`DifficultyGroupAi` 8 / `DifficultySelection` 9 / `ImpactBudget` 8)이
  모두 실행됐고 `DungeonGoldenDigestTests` 7건이 **재핀 없이** 통과했다.
- `[OBSERVED]` **WebGL 빌드·배포 완료**: `result=Succeeded size=70743703 errors=0`
  (120 MB 계약 내). `gh-pages` `b4ef488..6b56319`, 라이브
  <https://akillness.github.io/hongT/> 의 4개 런타임 아티팩트 SHA-256 이 로컬과 일치.
