...
## AMENDMENT #11 — 난이도 티어 (2026-08-08 추가)

출처: `docs/SIM_SPEC_HACKSLASH.md` §16.2 (`Assets/Scripts/Sim/DifficultySpec.cs`
가 유일한 구현 진실). 이 표는 사본이므로 수치를 바꿀 때는 스펙과 코드를 먼저
고치고 여기로 옮겨 적는다.

| 티어 (표시) | enum | 받는 피해 | 적 공격 쿨다운 | 동시 공격 상한 | 그룹 AI | 대기 링 반경 | 측후방 우선 |
|---|---|---|---|---|---|---|---|
| 입문 (Story) | 1 | ×0.65 | ×1.22 | 2 | off | ×1.00 | 1.00 |
| 보통 (Normal) | 0 | ×1.00 | ×1.00 | 무제한 | off | ×1.00 | 1.00 |
| 어려움 (Hard) | 2 | ×1.35 | ×0.84 | 3 | **on** | ×1.55 (117.8) | 0.75 |
| 악몽 (Nightmare) | 3 | ×1.70 | ×0.70 | 4 | **on** | ×1.35 (102.6) | 0.75 |

- 보통이 enum 0 인 것은 의도다 — `default(HackConfig)` 가 개정 이전 수치를 그대로
  재현해야 골든 다이제스트를 재핀하지 않는다.
- 대기 링 반경은 `SimConfig.EnemyAttackRange = 76` 기준 배수. 링 슬롯 8개,
  도착 허용 오차 16, 정면 판정 임계 -18(프로즌 계약 재사용).
- 던전 강하에만 적용된다. 아레나·프롤로그는 항상 보통이다.

### [OBSERVED] 관측치 (dotnet 격리 실행, 플레이어 정지 시나리오)

| 티어 | 최소 공격 간격(틱) | 동시 스윙 최대 | 사거리 내 최대(1800틱) | 평균 거리(1800틱) |
|---|---|---|---|---|
| 입문 | 93 | 2 | — | — |
| 보통 | 75 | 3 | 4 | 93.8 |
| 어려움 | 64 | 3 | 3 | 106.4 |
...
최소 공격 간격은 각 티어의 쿨다운 배수를 그대로 따라간다(스윙 애니메이션 시간이
포함돼 있어 비율이 정확히 배수와 같지는 않다). 어려움에서 사거리 안으로 몰려드는
적 수가 줄고 평균 거리가 늘어나는 것이 포위·교대가 실제로 작동한다는 증거다.

**아직 측정하지 않은 것**: 각 티어의 실플레이 클리어율·재미. 플레이테스트 필요.
