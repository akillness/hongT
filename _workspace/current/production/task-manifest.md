...
**이 표가 승인된 범위 전체다.** 승인되지 않은 추가 제안은 바로 아래 §범위
추가 제안에 있고, 표에 올리지 않는다 — 동결된 범위표에 미승인 항목을 섞으면
승인된 것처럼 읽힌다.

### 범위 추가 제안 — 미승인

아래는 **deep-interview가 동결한 범위 밖**이다(모호도 15% 확정 시점에
없었다). W1–W4가 받은 것과 같은 승인 경로가 필요하고, 승인 전에는 구현
대상이 아니다.

| 수 | 내용 | 심 | 골든 | 상태 |
|---|---|---|---|---|
| **W5 명령 각인** | 레벨업 오퍼에서 각인 이름을 타이핑해 풀 전체에서 호출 (기존 커맨드 콘솔 확장) | **없음** — View 사전 대조 후 기존 `SimInput.GrowthChoice` 정수 범위 확장 | 미옵트인 무이동 | **미승인 — 설계 초안** (`design/dungeon-entry-fun-spec.md` §18) |

선행 조건 2건 (§18이 명시):

| # | 조건 | 소유 |
|---|---|---|
| 1 | entry 18 인접 가격 확인 — 시간 비용이 선택 품질 상승분을 가격하는가 | pm |
| 2 | 각인 A/B 면 이름 10개를 View 리터럴로 추가 + `tools/gen_hud_font.sh` 재실행 + 폰트 동반 커밋 | programmer |

제안 사유: 사용자 요구(2026-08-12) + §17 부채 1번(초반 선택 압력 0.400)의
구조적 해답. 타이핑은 제시 수를 1로 낮춰 풀 크기를 늘리지 않고 압력을
밴드 위로 올린다 — G2 무변경 목표와 충돌하지 않는다.
