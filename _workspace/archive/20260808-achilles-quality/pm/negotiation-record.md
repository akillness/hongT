...
## cycle 9 G5 재측정 (run-id 20260808-achilles-quality)

cycle 8에서 배포 로비의 지출 UI 도달률이 `0% -> 100%`로 바뀌어 기존 T5
세션 분모는 비교 불가능해졌다. [OBSERVED: `retrospectives/cycle-8-retrospective.md`
G4/D-9] 아래 엔트리는 런타임 가격이나 보상을 바꾸지 않고 측정 계약만 건드리지만,
`10–20` 세션 판정을 바꾸므로 수치 협상 대상으로 기록한다. [TARGET]

### entry 17 — T5 세션 분모와 구매 정책

```yaml
entry: 17
revenue_point: "없음 — NAN 2026 제출 빌드는 비수익화; G5 진행 공정성 측정"
balance_number: |
  steady.parity_sessions_band [10,20].
  도달점 = weapon/lantern/cloak 전부 ProgressionGuide.EquipCap(T5).
  분모 = 도달 가능한 로비에서 시작해 clear 또는 defeat로 정산된 dungeon attempt.
designer_bound: |
  cycle-9용 새 분모·구매 정책 수치안 미제출.
  기존 하니스 상한(10–20)은 유지하되 새 진행 가격·보상 수치는 승인하지 않음.
pm_bound: |
  런별 실제 RunDigest.Relics + first-clear/pact 정산 + ICampaignSnapshot 최종 티어를
  순서대로 적용한다. 로비에서는 ProgressionGuide 가격을 읽어 가장 싼 장비 단계를
  반복 구매하고 동가이면 weapon -> lantern -> cloak 순서로 판정한다.
  각인 지출은 canonical T5 트랙에서 0이며 실제 발생하면 protocol deviation으로 보고한다.
agreed: escalated
status: pending
blocked_on: |
  game-designer의 측정 계약 검토와 QA의 5아키타입 x pact off/on x mastery 분리
  세션 리플레이. 결과가 없으므로 cycle-9 G5 PASS 서명 불가.
requested_signatures: [game-designer, game-pm]
signed: []
```

`signed: []`는 의도적이다. [OBSERVED] 기존 임계값 유지에 대한 일반 동의는 새
세션 분모와 구매 정책에 대한 공동 서명이 아니며, 이를 서명으로 승격하지 않는다.
[TARGET]
