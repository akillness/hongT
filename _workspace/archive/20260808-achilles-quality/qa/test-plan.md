...
| 4 | **companion-commander** | Player runs the same proven kiter band. When an active gimmick anchor is within 200 px it queues `CompanionHoldQueued`; otherwise it queues `CompanionRecallQueued`, only on state transitions. | Companion pathing vs blockers (pillar-class gimmicks), hold/recall determinism near hazard zones (HackSimTests §companion invariants extended to new gimmicks). |
...
# QA Test Plan — Five-cycle action-quality program (run-id 20260808-achilles-quality)

2026-08-08 · game-qa · cycle 9 enters **Stage 2 Phase 2a** under the cycle-8
retrospective. This plan does not restart Stage 1 and does not replace the current concept.
Achilles calibrates action-RPG play concepts for cycles 10–13 only.

## 0. Entry order and stop rules

1. **G8 first**: score the existing HongT novelty candidate before retuning. Cycle 8 made
   this a Stage-2 entry condition; it cannot be carried a sixth time.
2. Run the cycle-9 archetype/exploit matrix on the current concept.
3. Recompute G5 growth/fairness after spend-UI reachability changed from 0% to 100%.
4. Verify one implementation owner for equipment/sigil state.
5. Re-verify G2/G3/G7, then proceed to cycles 10–13 action-quality work.

Stop immediately and file/block on:

- any open S1 defect;
- a missing evidence path for the gate being judged;
- a digest difference on an inherited Arena/Prologue/Dungeon golden row;
- a G8 session with fewer than 5 independent scored testers;
- any reused identical deterministic script counted as a new sample;
- any NAN public packet that still claims 166/166 or six final stages at submission.

## 1. Archetype rotation

Each automated archetype is a declared pure input policy. Each human session uses the same
goal/policy statement. QA records stage, difficulty, loadout, input-script id, outcome,
TTK, damage source, relic delta and loop re-entry.

| ID | Archetype | Distinct strategy | Required exploit pressure |
|---|---|---|---|
| A1 | combo-rusher | close immediately; maximize Space chain/finisher uptime | active-window boundary, dash cancel, boss phase skip |
| A2 | kiter-dodger | maintain range; dash only against committed threats | clamp safe spots, telegraph recognition, no-damage timeout |
| A3 | defensive-turtle | prioritize survival, shield/ward and low-risk openings | invulnerability/ward chaining, zero-risk stall, healing/rearm |
| A4 | skill-economist | maximize damage per oil/cooldown, avoid basic attacks when possible | free-cast, double debit, cooldown reset, dominant pair EV |
| A5 | companion-commander | hold/recall/focus positioning; player damage secondary | leash/target-lock loop, hazard outsourcing, inactive-mode leakage |
| A6 | low-APM-casual | ≤2 action decisions/s; no animation-edge timing | accessibility, readability, Story-tier viability |
| A7 | growth-optimizer | choose stage/spend order for earliest canonical T5 parity | UI reachability vs affordability, save/reload, duplicate owner |

G3 sampling contract:

```yaml
archetypes_tested: 7
distinct_input_scripts_per_matchup_cell: 20
viable_win_rate_band: [0.45, 0.55]
independently_viable_min: 3
optimal_play_dominance_max: 0.50
```

A matchup cell is `(archetype, stage, difficulty, declared loadout)`. Its win rate is
`clears / 20 distinct scripts`. Scripts differ by a predeclared input schedule (route,
skill order, engagement timing); replaying one schedule only checks determinism and never
increases the denominator. Dominance is `wins by the most successful strategy / wins by
all strategies in the compared optimal set`.

## 2. Simulation matrix and exploit probes

### 2.1 Cycle-9 core matrix

- Stages: all nine logical `StageCatalog` entries.
- Difficulty: Story, Normal, Hard, Nightmare for Dungeon; Prologue/Training/Arena retain
  their contracted mode behavior.
- Loadouts: fresh/0, contested 2/1/3, capped 5/5/5, plus each archetype’s declared build.
- Run length: until clear/defeat, or 5 minutes; a timeout is not silently converted to a win.
- Determinism replay: every failing or band-edge script is replayed 10 times; all snapshots,
  events, outcome tick and digest must be identical.

### 2.2 Exploit register probes

| Probe | Exact injection | Measured fields | Failure rule |
|---|---|---|---|
| X1 commit/cancel | Space, Shift and Q/E/R/F at −1/0/+1 tick around active-window boundaries | accepted action, damage, oil, cooldown, pose, digest | duplicate damage, free cast, double debit, or sim/pose disagreement |
| X2 input flood | 30 Hz synthetic action inputs for 60 s | queued/accepted events, charges, cooldowns, allocations | accepted count differs from charge/cooldown ledger; unbounded queue/growth |
| X3 clamp/safe spot | sweep the full playable boundary at 16 u spacing, then idle 1,800 ticks at candidates | incoming attacks, hazard hits, enemy path distance | a reachable point prevents every contracted threat while damage/reward continues |
| X4 hazard outsourcing | lure enemies through current/wall/vent while player deals 0 direct damage | enemy damage source, kills, credit, player risk, TTK, EV | risk-free clear or pair EV >1.3× median |
| X5 pylon precedence | strike protected enemy and pylon on adjacent ticks in both orders | pylon HP, enemy HP, shield state, hit credit | order-dependent double hit, shield bypass, reward duplication |
| X6 boss thresholds | burst across 50% and 20% HP at −1/0/+1 tick | phase event count, plates, boss HP, rewards | skipped/duplicate phase event or duplicate terminal reward |
| X7 mode leakage | send Dungeon dash/skills/companion inputs in Prologue, Training and Arena | digest, economy, persistent blob, event flags | any prohibited command changes state |
| X8 modal/timeScale | open/pause/close guidance, command console, growth and terminal panels during hazard/attack ticks | sim tick delta, damage, duplicate panel, queued input | ownership conflict, invisible sim advance where pause promised, replayed input |
| X9 save/reload economy | reload before/after purchase, equip, sigil, first-clear and trial result | relics, tiers, active sigil, masks, trial tiers | gain/duplication/loss outside the declared atomic transition |
| X10 difficulty swap | change selector before run, during run, after terminal and after reload | active tier, snapshot multipliers, persistent id | mid-run mutation, enum-integer remap, corrupted-id crash |
| X11 comeback rearm | cross threshold, heal above rearm, cross again; also hover below threshold | activations, reversals, cap, cooldown, health | run cap/cooldown breach or reversal probability >30% |
| X12 UI duplicate owner | perform identical equip/sigil action through each surviving UI surface | save blob, cost, tier, active sigil, rendered state | any divergence; cycle-9 cannot close without one named owner |

Every reproducible break uses the harness row schema in `qa/exploit-register.md`:
`id | severity | archetype | repro steps | measured value vs band | status | broadcast-at`.
At audit start that path was missing; no exploit claim is complete until the register exists.

## 3. Action-feel measurement

Cycle 9 measures existing actions only: basic/combo attack, Dungeon dash, Q/E/R/F skills,
Arena skills and companion commands. Guard/perfect-counter is an Achilles benchmark verb,
not a current HongT verb; it is neither faked nor treated as a cycle-9 defect. If adopted
later, it first requires a deterministic input/event contract and joins the same boundary matrix.

### 3.1 Timestamp schema

For 30 trials per action and browser mode, capture:

```text
trial_id, mode, action, input_ts, accepted_sim_tick, pose_onset_ts,
contact_sim_tick, vfx_onset_ts, audio_onset_ts, result_visible_ts,
fps, reduced_motion, outcome
```

Derived values:

- `input_to_pose_ms = pose_onset_ts - input_ts`
- `contact_to_vfx_ms = vfx_onset_ts - contact_sim_time`
- `contact_to_audio_ms = audio_onset_ts - contact_sim_time`
- `contact_to_result_ms = result_visible_ts - contact_sim_time`

Thresholds:

```yaml
g4_feedback_latency_ms_max: 100
trials_per_action_mode_min: 30
missed_required_feedback: 0
duplicate_required_feedback: 0
deterministic_tick_replay_match_pct: 100
```

The existing attack motion anchor is normalized time 0.22 windup, 0.53 contact and 0.78
follow-through (`qa/swing-motion/mcp-playmode-verification.md`). It is a pose baseline,
not proof of shipped browser latency.

### 3.2 Telegraph/response checks

- Boss telegraph remains 0.80 s in every phase; record recognition and action choice.
- Tide-current telegraph 0.80 s and ash-wall telegraph 1.50 s are exact contract anchors.
- Probe dodge at one simulation tick before/on/after contact.
- Reduced-motion removes time stop but must retain a readable outcome signal within 100 ms.
- A timing value without input/event timestamps and a trace/video path is `UNKNOWN`.

## 4. Immersion and impression rubric

Run ≥5 independent scored sessions for each scene: Dungeon ordinary wave, Dungeon named
boss/finish, Prologue reveal, each submitted Training trial scene, and Arena crowd/terminal.
Each ballot contains five integer dimensions.

| Score | Observable anchor |
|---|---|
| 1 | Tester misses or misattributes the event/outcome. |
| 2 | Tester identifies it only after the decision window or sees conflicting feedback. |
| 3 | Tester identifies event and outcome in-window but needs one recheck or prompt. |
| 4 | Tester identifies event, response and outcome on first viewing without external explanation. |
| 5 | Score-4 clarity plus tester can recall the counter/timing and scene identity after the scene. |

Dimensions:

1. input response;
2. threat readability;
3. hit/outcome clarity;
4. scene/location identity;
5. audio/visual cohesion.

G4 computation:

- scene score = median of all dimension scores across that scene’s ballots;
- aggregate = median of scene scores;
- every required scene median and aggregate must be ≥4.0/5;
- every effect-feedback spot-check must be ≤100 ms;
- unresolved readability complaints at S1/S2 must be 0.

G8 uses a separate **five-rater blind panel** over final Playwriter captures. Randomize capture
order, hide candidate/benchmark labels, and do not describe the intended novelty. First ask
each rater, unprompted, which single element they would describe; then score memorability
with the same 1–5 anchors. At least one candidate needs median ≥4/5 **and** frequency ≤2
among ≥5 comparable titles. Retain all five raw ballots, capture order and capture paths;
frequency evidence never substitutes for the score.

## 5. Growth, comeback and fairness simulation

### 5.1 Fresh-save parity ledger

Use the canonical PM route contract in `pm/reward-bands.md`; do not substitute the seven
G3 policies for the five named G5 pilots.

1. pilots: `melee-rusher`, `kiter`, `skill-spammer`, `companion-commander`,
   `pacifist-dodger`;
2. prove `PrologueDone=true` and reachable Sanctum before the first counted leg;
3. count only Dungeon legs settled by clear or defeat; exclude Prologue, Training, Arena,
   menu-only visits and manual abandon;
4. execute routes A–E exactly: A=lobby every leg/pact off; B=all Ember Rest continuations/
   pact off; C=A/pact on for eligible replays; D=B/pact on; E=A plus the actual one-time
   training mastery +2 occurrence;
5. before nine clears follow `ProgressionGuide.NextTarget`; afterward replay all nine catalog
   entries in order;
6. join live `RunDigest.Relics`, settlement multiplier, first-clear component, final rank
   transitions, persisted state and purchases; never infer sessions from average income;
7. at lobby spend repeatedly on the cheapest next equipment step, tie-breaking
   weapon→lantern→cloak; spend 0 on sigils in the canonical track;
8. stop at all-three T5 or session 21 and report exact `N_T5` for every pilot-route plus
   minimum, median and maximum.

Pass condition: every pilot-route used by the parity claim has `10 <= N_T5 <= 20`.
`N_T5 <10`, `>20`, or not reached by 20 is a measured failure. Reachability and economic
affordability remain separate fields.

### 5.2 Equal-skill paid/free check

PM declares monetization, real-money purchase and paid shortcuts disabled, with a current-build
identity delta of 0 pp by inference. QA must prove the premise:

- audit the candidate for callable/visible offer, checkout, premium currency, paid continue
  and paid-power state;
- assert `paid_offer_visible=false` and `paid_power_applied=false` in every joined row and
  all 24 browser sessions;
- if any paid state is constructible, pair identical stage/difficulty/pilot/input scripts,
  changing only paid/free state.

Compute `delta_pp = paid_win_pct - free_win_pct`; absolute delta must be ≤5 pp. Identity
0 pp does not pass until the absence audit is measured.

### 5.3 Comeback check

For every peril activation opportunity in the
`5 pilots × 9 stages × reference loadouts` grid, replay the identical config/input trace
with all peril clauses off, then with only activation `i` enabled. Count a reversal only
when the all-off run fails and the isolated-activation run clears.
`instant_reversal_probability = isolated_reversals / activation_opportunities` must be
≤0.30; also verify ≤2 activations/run, re-arm only after health ≥50%, and no simultaneous
clauses. Deterministic paired replays replace RNG sampling; they do not waive the cap.

### 5.4 Revenue-signature audit

Enumerate every current reward/revenue point and join it to `pm/negotiation-record.md`.
Entry 17 is currently `escalated`, `pending`, `signed: []`; G5 remains FAIL until QA emits
the joined route rows and designer/PM sign the measurement denominator and purchase policy.
The old 25% first-clear ratio is retained as history and is superseded as the comeback
definition; it is not the cycle-9 decision rule.

## 6. Browser-mode matrix

Build/session commands are defined for reproducibility; this planning update did not execute
Unity or a build.
...
| Mode | URL | Required setup | 90-second/terminal-path assertions | Evidence basename |
|---|---|---|---|---|
| Dungeon | `http://127.0.0.1:4173/?mode=campaign&stage=cinder-span&intro=off` | valid `prologueDone` + unlocked stage; log exact seed blob | combo/dash/QERF; wave banner; hazard; terminal; state write; 0 console/page/request errors | `c9-browser-dungeon-*` |
| Prologue | `http://127.0.0.1:4173/?mode=prologue&intro=off` | fresh save | movement→attack→oil→wave tutorial; Dungeon-only inputs inert; clear writes `prologueDone`; 0 errors | `c9-browser-prologue-*` |
| Training | `http://127.0.0.1:4173/?mode=training&trial=<id>&tier=<n>&intro=off` | `prologueDone=true`; declare trial id/tier | 60 s timer; no enemies/kills/relics; mechanic visible; tier result write; prohibited mode inputs inert; 0 errors | `c9-browser-training-*` |
| Arena | `http://127.0.0.1:4173/?mode=arena&intro=off` | clean mode state | Arena controls only; inherited golden behavior; wave/game-over path; Dungeon-only inputs inert; 0 errors | `c9-browser-arena-*` |

Execute each row at all six presentation/control cells:

| Cell | Outer viewport | Input |
|---|---:|---|
| D-K | 1280×853 | keyboard |
| D-T | 1280×853 | touch |
| F-K | 375×667 support floor | keyboard |
| F-T | 375×667 support floor | touch |
| R-K | 667×375 rotated | keyboard |
| R-T | 667×375 rotated | touch |

Total: 4 modes × 6 cells = **24 browser sessions**. Record actual canvas CSS size, DPR,
WebGL context, request failures, console errors, page errors, screenshots at boot/action/
terminal, localStorage before/after, and the action timestamp trace. Every required control
must be contained and reachable; every touch target at the support floor must be ≥44 CSS px.
Rotation-specific intentional blockers must be visible and leave no hidden active control.

## 7. G1–G8 measurement paths

All thresholds come from
`skill://game-studio-harness/references/quality-gates.md`. The command/session and artifact
columns are mandatory; a blank cell is FAIL.

| Gate | Exact threshold | Executable or session path | Required artifacts |
|---|---|---|---|
| G1 | 0 unwaived lore violations; 100% shipped strings/effects/scenarios trace to `design/worldview.md`. | Session `C9-G1-CONTENT-AUDIT`: enumerate every player-visible string/effect/scenario in the candidate; assign worldview anchor or violation/waiver; verify all 24 browser sessions for rendering. | `qa/gate-measurements.md#g1`, content manifest, violation list, browser screenshots; formula `traced/total ×100`. |
| G2 | 100% mechanics in balance sheet; matchup win rates 45–55%; TTK within ±15% target; no pair >1.3× median EV. | `bash tools/unity_batch.sh tests`; then session `C9-G2-SIM-MATRIX` using §2.1, event export and all-pair enumeration. | Unity test XML/log; sim rows; TTK table; EV table; `qa/gate-measurements.md#g2`. |
| G3 | ≥5 archetypes tested; ≥3 independently viable in band with distinct strategies; no archetype >50% optimal-play dominance. | Session `C9-G3-ARCHETYPE-ROTATION`: §1 seven policies × declared matchup cells, 20 distinct scripts/cell. | `qa/playtest-report.md` per-script rows, dominance calculation, `qa/gate-measurements.md#g3`. |
| G4 | median immersion ≥4.0/5; feedback latency ≤100 ms; 0 unresolved S1/S2 readability complaints. | Sessions `C9-G4-IMMERSION` (§4) and `C9-G4-LATENCY` (§3), across the 24-cell browser matrix. | raw ballots, trace CSV/JSON, video/screenshots, defect register, `qa/gate-measurements.md#g4`. |
| G5 | paid/free delta ≤5 pp; reversal probability ≤30% with cap/cooldown; free parity 10–20 sessions; every revenue point signed. | Sessions `C9-G5-GROWTH`, `C9-G5-COMEBACK`, `C9-G5-FAIRNESS` using §5; audit negotiation records. | fresh-save ledgers, activation rows, paired-run/waiver evidence, signed-entry join, `qa/gate-measurements.md#g5`. |
| G6 | telemetry fields 100%; rollback tested once; release checklist 100%; p95 frame ≤16.7 ms; long-frame ratio <0.5%; memory stable over 30 min; input ≤100 ms. | `bash tools/unity_batch.sh tests`; `bash tools/unity_batch.sh build`; session `C13-G6-SOAK` in shipped WebGL; session `C13-G6-ROLLBACK`. | test/build logs, perf trace, 30-min memory series, telemetry audit, rollback transcript, release checklist, `qa/gate-measurements.md#g6`. Memory stable = post-warmup end ≤105% of start and no positive retained-memory slope >1% of baseline per 10 min. |
| G7 | Harness: loop 30–180 s, ≥3 actions, ≥1 reward, voluntary re-entry ≥70%. Cycle-9 L1 is stricter: 30–90 s, ≥4 actions, ≥1 reward; L2 boss 30–180 s, every phase ≥2.17 s, answer identification ≥70%. | Session `C9-G7-L1`: segment `WaveStarted`→next `WaveStarted`/`StageCleared`/`GameOver`; pair a <30 s nonterminal wave with the next, and fail >90 s unless boss. Session `C9-G7-L2`: boss-wave `WaveStarted`→terminal including setup. Then ≥10 human sessions with no re-entry prompt. | Per-segment start/end/duration/actions/rewards, phase durations, intended action + room-rule answers, raw re-entry decisions, `qa/playtest-report.md`, `qa/gate-measurements.md#g7`. |
| G8 | ≥1 element in ≤2 of ≥5 surveyed titles and QA impression median ≥4/5. | **First session** `C9-G8-ENTRY`: five-rater blind panel over randomized final Playwriter captures before retune; independently verify frequency cells/provenance. | five raw ballots, randomized capture order/paths, median calculation, frequency table and source labels, `qa/gate-measurements.md#g8`. |

### Automated runner identity

`tools/unity_batch.sh tests` runs the entire Unity EditMode suite and writes timestamped
`_workspace/current/engineering/unity-logs/test-results-*.xml` plus `tests-*.log`.
`tools/unity_batch.sh build` writes a timestamped build log. Copy exact output paths into
the gate row; “tests passed” without the XML/log path is not evidence.

## 8. Regression and public-beat audit

Inherited regression:

- arena/prologue/invariant/new-dungeon golden digests: 100% identical unless an approved
  contract amendment names and repins the exact rows;
- deterministic Hard/Nightmare replay: 100% identical snapshots/events;
- Prologue and Training: no Dungeon-only command/economy leakage;
- Training: 0 enemies, 0 kills, 0 relics for the contracted 60-second trial;
- browser: 24/24 session rows, 0 console errors, 0 page errors, 0 failed required requests;
- touch: every required target ≥44 CSS px at 375×667;
- NAN Markdown/PDF: 0 stale occurrences of `166/166`, six-final-stage claims, or any other
  numeric claim contradicted by accepted gate evidence.
...
Expected before public-beat sign-off: no stale factual matches; any intentionally historical
match must carry an explicit date/context and must not appear as current submission status.

## 9. Cycle-9 blocking defects and missing measurements

| ID | Reproducible fact | Numeric status | Cycle-9 disposition |
|---|---|---|---|
| C9-B1 | `design/novelty-scorecard.md` shows every candidate impression as `미측정`; cycle-8 retrospective names five carry-overs. | G8 score `UNKNOWN`; required median ≥4/5, N≥5. | **Blocks Stage-2 entry. Run `C9-G8-ENTRY` first.** |
| C9-B2 | `qa/gate-measurements.md#g5` says spend reachability changed 0%→100% and sessions-to-T5 needs rerun. | parity `UNKNOWN`; required 10–20 sessions. | Blocks G5 and cycle-9 Stage-2 exit. |
| C9-B3 | `pm/negotiation-record.md` entry 17 is `escalated`, `pending`, `signed: []`; current local telemetry does not join route, reachability, reward components, rank transitions and purchases. | required signatures 0/2; joined replay rows 0; G5 pending. | Blocks G5 until five pilots × routes A–E replay, joined evidence, comeback/paid-path checks and signatures. |
| C9-B4 | at audit start, `qa/exploit-register.md`, `qa/defect-register.md`, and `qa/regression-matrix.md` were absent. | required evidence paths: 0/3 present at audit. | Missing path = FAIL for dependent gates; materialize before verdict. |
| C9-B5 | cycle-8 expires duplicate equipment/sigil implementations in cycle 9. | implementations observed: 2; allowed canonical owners: 1. | Run X12; name one survivor before cycle close. |
| C9-B6 | NAN current status says 166/166 and six/final stage while accepted evidence says 808/808 and nine logical stages. | two fact families stale. | Blocks NAN 2026 final packet, not a code gate; QA reports, docs owner corrects/regenerates. |
| C9-B7 | current G4 evidence measures lobby reachability but not the harness-required four-mode immersion median or shipped action latency distribution. | immersion `UNKNOWN`; latency `UNKNOWN`; required ≥4.0 and ≤100 ms. | Does not invent a cycle-9 feature; must close by the relevant Stage-3/five-cycle gate. |

No PASS may be inferred from a previous adjective, a screenshot without a session row, an
aggregate test count without its XML/log, or a frequency table without an impression ballot.
