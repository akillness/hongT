    ...
    public sealed class LobbyView : MonoBehaviour
    {
        ...
        public void Refresh(CampaignData data)
        {
            ...
            for (var i = 0; i < StageCatalog.Entries.Count; i++)
            {
                ...
                // Route STATE is the map's job; the card keeps only what the map
                // cannot say. Two of the four words were pure repetition and are
                // gone:
                //   강하 가능 -> the enabled 강하 button IS the statement, and the
                //               map header already names it as "다음 X".
                ...
                // Two stay, because nothing else on a card carries them:
                //   정화 완료 -> a cleared card keeps a live 강하 button for
                //               replay, so it is otherwise indistinguishable,
                //               and the compact map has no room for the epithet.
                //   다음 재판 -> N2 disclosure. The survey found 0 of 6 comparable
                //               titles RECOMMENDING a next action and Hades
                //               excluding it on purpose; disclosing which door is
                //               next is what the genre does instead. It marks ONE
                //               card, so it is a marker among nine, not a state
                //               repeated nine times — which is what the
                //               de-duplication pass was actually about.
                var isTarget = target.Kind == GuideTargetKind.Stage && target.Index == i;
                _stageStatus[i].text = cleared ? "정화 완료"
                    : isTarget ? "다음 재판" : string.Empty;
                _stageStatus[i].color = cleared ? Gold : Ember;
        ...
        void BuildActStages(Transform body, int act)
        {
            ...
            for (var slot = 0; slot < ProgressionGuide.StagesPerAct; slot++)
            {
                ...
                // Empty until Refresh: only a CLEARED or TARGET card carries a
                // word here (see the de-duplication note in Refresh). Building
                // it with "잠김" would flash a removed state for one frame.
