...
영상, 게임 코드와 검증 하니스, 기술 검증용 자동화 플레이 캡처까지 AI 워크플로
위에서 만들어졌습니다. 제출용 플레이 영상은 사람이 직접 조작해 별도로 촬영합니다.
...
| 기술 검증 영상 | Playwright + 자동화 CDP 입력 녹화 | 로컬 후보 실프레임 캡처; 제출 영상은 사람 플레이로 별도 촬영 (§6.3) |
...
- 최근 전체 EditMode 결과: **812 / 812 passed, 0 failed**
  (`_workspace/current/engineering/unity-logs/test-results-013939.xml`).
  중간 마일스톤은 371/371 → 383/383 → 737/737 → 785/785 → 800/800 →
  808/808 → 811/811 → 812/812로 증가했으며, 어느 단계에서도 실패를 남긴 채 진행하지 않았습니다.
...
- **한글 HUD 폰트**: `tools/gen_hud_font.sh`가 `Assets/Scripts/View/*.cs`의
  ...
  과거 `"Companion cadence "` 뒤에 U+2212를 둔 리터럴에서 해당 글리프가
  charset에 들어가지 못했는데, 커버리지 검사도 **같은 charset**과 비교하다 보니
  FULL이라고 출력했습니다. 현재 런타임 표기는 ASCII `-`로 교체했습니다. "자기
  사각지대와 비교하는 검사기는 검사기가 아니다" — 두 규칙의 합집합으로 수정했습니다.
...
## 6.3 플레이 영상 검증과 제출 경계

대회 규정은 *"AI를 이용한 조작·합성이나 타인 영상의 도용은 불가"*이며
*"실제 플레이 화면 그대로"*를 요구합니다. 아래 자동화 캡처는 합성·보간 없이
실제 렌더 프레임을 기록하지만, 입력 주체가 자동화 스크립트이므로 제출하지 않습니다.
제출본은 동일 대본을 사람이 직접 조작해 별도로 촬영합니다.

**채택하지 않은 방법**: 스크린샷을 이어붙이고 가상 커서·줌을 얹는
screenshot-to-video 렌더링은 규정이 금지하는 합성에 해당하므로 배제했습니다.

**기술 검증에 채택한 방법** (`tools/video/capture-unity-play.mjs`):

1. 실제 Chromium이 로컬 최종 WebGL 후보 `http://127.0.0.1:9876/`를 로드
2. 자동화 드라이버가 **CDP 입력 도메인**으로 키를 주입 — 게임 상태를 직접
   변경하지 않지만, 입력 주체는 물리 키보드를 누르는 사람이 아니라 스크립트
3. 브라우저가 실제로 렌더링한 프레임을 그대로 녹화 — 보간·리타이밍·합성·생성 없음
...
| Playwright | Apache-2.0 | 기술 검증용 자동화 플레이 캡처 드라이버 |
| ffmpeg (CompressO 번들 및 시스템) | LGPL/GPL (빌드 구성별) | 검증 영상 조립·압축 |
| CompressO | **§7.4 확인 필요 8** | 기술 검증 영상 압축; 제출본도 사람 플레이 촬영 후 동일 압축 설정 사용 가능 |
...
- 변이 검증이 있었기에 "812/812 초록"이 의미 있는 문장이 됐습니다.
