...
| 4 | `_revealReturnTimer` 2.6 s 만료 → `EnterLobby()` | 로비로 안착. 성장/장비/군단 탭과 6단계 출정 카드가 드러난다. | 로비 테마 진입 |
...
| 증언의 우물 (Witness Well) | `(0.45, 0.78, 1.0)` 우물의 청록 | 증언의 우물은 대답보다 먼저, 무엇을 잊었는지 묻습니다. |
...
- **전이 연출** `[TARGET]`: 깨진 거울 파편 사이로 카메라가 하강, 우물의
  청록(`0.45,0.78,1.0`) 수면 반사가 서서히 드러난다.
...
- **비주얼**: `BossCommander` / `shadow-commander-boss`. 성당 Tint
  `(0.56,0.40,1.0)` Scale 1.1 → 우물 Tint `(0.45,0.78,1.0)` Scale 1.12.
- **등장 (BossEntry)**:
