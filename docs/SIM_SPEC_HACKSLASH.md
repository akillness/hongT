...
| 16 | 진행 네비게이션 (아코디언 · 잠금 사유 · 다음 재판 · 각인 컨텍스트) | 본 문서 "AMENDMENT #8 — 진행 네비게이션" (로컬 절 번호 **§16–§20**) | frozen — 순수 뷰. 신규 저장 필드 0, 신규 심 수치 0, 골든 무이동. 도착 번호 #8은 동료 시그니처가 이미 점유 |
| 17 | 인게임 안내 (23종 · 정지 8 · 토스트 15 · 코덱스) | 본 문서 "AMENDMENT #9 — 인게임 안내" (로컬 절 번호 **§21–§27**) | frozen — 저장 필드 1개 추가(`GuidanceSeen`, v6 additive). 도착 번호 #9는 모멘텀 게이지가 이미 점유 |
| 18 | 안내 탭 가독성 (게임설명 · 그룹 페이징 · 스택 아이콘) | 코드 주석 "AMENDMENT #10" (`View/HudViewCodex.cs`, `View/HudView.cs`) — 스펙 절 없음 | frozen — 순수 뷰, 심·저장 무변경. 도착 번호 #10은 훈련장·돌발이 이미 점유 |
...
## 개정 번호 병합 노트 (2026-08-08, dungen 레인 합류)

`dungen` 레인이 세 증보를 #8·#9·#10으로 붙인 채 도착했고, main은 그 사이
#8(동료 시그니처)·#9(모멘텀)·#10(훈련장·돌발)을 이미 소비했다. 부록 A의 규칙을
그대로 적용한다 — **기존 헤딩 문자열은 재작성하지 않는다.** 코드 주석·커밋
메시지·QA 리포트가 도착 헤딩을 인용하고 있고, 문자열을 밀면 그 인용이 전부
끊긴다. 원장이 유일한 진실이다:

| 도착 헤딩 | canonical | 로컬 절 번호 | 충돌 상대 |
|---|---|---|---|
| `AMENDMENT #8 — 진행 네비게이션` | **16** | §16–§20 | 로컬 §16이 Amendment #11의 §16.x와 겹침 |
| `AMENDMENT #9 — 인게임 안내` | **17** | §21–§27 | 로컬 §17 없음 (Amendment #13이 §17.x 사용) |
| `AMENDMENT #10 — 안내 탭 가독성` | **18** | 없음 (순수 뷰) | — |

로컬 절 번호가 겹치는 것은 새 사고가 아니다: 부록 A가 Amendment #11에 대해
"로컬 절 번호 §16"을 이미 그렇게 기록하고 있고, 로컬 번호는 자기 증보 안에서만
의미를 갖는다.

`dungen` 레인이 훈련장·돌발을 **#7**로 부르던 것도 같은 사고였다. 그 작업은
main의 **#10**과 동일한 내용이며, 이 문서에는 main의 것만 남는다.

---
---

## AMENDMENT #8 — 진행 네비게이션 (run-id 20260807-progression-navigation)

**심 수치 변경 0. 저장 필드 추가 0. 이 개정은 순수 뷰다.**

기존 개정과 성격이 다르므로 그 사실을 먼저 못 박는다. #6(각인)과 #7(훈련장·
돌발)은 심에 조항을 더했다. #8은 **이미 저장된 상태를 화면에 옮기기만 한다** —
`ClearedMask`, `PrologueDone`, `PrereqId`, `TrialTiers`, `Relics`, `Points`가
전부 답을 갖고 있었고 화면에만 없었다. 따라서 골든 디제스트는 한 행도 움직이면
안 된다. 움직이면 그것이 결함이다.

### §16 진행 지목 (Progression Target)

`ProgressionGuide.NextTarget(in CampaignData)` — 순수 함수, 결정론.
...
- 해금 판정은 `StageCatalog.IsUnlocked`를 **호출**한다. 규칙을 복제하지 않는다.
- 인덱스가 전순서이므로 동률이 없다 — 슬롯 수는 구조적으로 0 또는 1.
- 4번 `None`은 **정당한 종단**이다. 전 구역 정화 + 숙달 완료 상태이며,
  "정확히 1개" 불변식은 1~3에만 적용된다.
- 손상 세이브가 도달 불가 `ClearedMask`를 만들 수 있으므로 검증은
  **1024 상태 전수**(PrologueDone 2 × ClearedMask 512)여야 한다.

### §17 잠금 사유 (Lock Reason)

`IsUnlocked == false`의 원인은 코드상 정확히 2가지다. 사유 문자열은 그 분기를
**명명**할 뿐 재유도하지 않는다.

| 사유 | 카드 서브라인 |
|---|---|
| `PrologueIncomplete` | `{별칭} • 점화 훈련 필요` |
| `PrerequisiteUncleared` | `{별칭} • 선행: {선행 한글명}` |
| 해금·미클리어 | `{별칭} • 보상: {동행명}` |
| 클리어 | `{별칭}` |

별칭이 항상 선두인 것은 계약이다 — 별칭이 예고이고, 예고를 잃은 잠긴 카드는
가르치기를 멈춘다.

**대비 계약**: 잠긴 카드는 `alpha 0.45`다. 이 뒤에서는 로비 팔레트 전체가
AA 4.5:1을 통과하지 못한다(골드 3.07 / 시안 2.31 / InkDim 2.48 / 잠금회색 1.71).
따라서 사유 라벨은 `CanvasGroup { alpha = 1, ignoreParentGroups = true }`로
감쇠 밖에 둔다. 색은 InkDim, 합성 후 **8.69:1**.
색 교체로는 해결 불가 — 이것은 배치 계약이지 색 계약이 아니다.

### §18 아코디언 (4그룹)

worldview §공간 계보의 3부작 + 훈련장. 그룹 정의는 **worldview가 유일 출처**이며
`StageCatalog.Entries.Count == ActCount × StagesPerAct`가 깨지면
`ProgressionGuide.ActOf`의 `index / 3`이 조용히 틀린다 — 테스트가 명시적으로 잡는다.
...
- **정확히 한 그룹만 펼쳐진다.** 높이 예산을 보장하는 유일한 제약.
- 기본 펼침 = 지목 대상이 속한 그룹. 헤더 탭이 핀을 걸고, `Hide()`가 해제한다.
- 폴드 상태가 컨트롤을 비활성화하므로 **터치 플로어 감사는 4상태 합집합**으로
  측정해야 한다. 단일 상태 감사는 훈련장 8컨트롤을 놓친다.

### §19 배지와 컨텍스트 (negotiation entry 10/11)

- 유물 탭(장비/각인)은 **최저비용 항목이 속한 쪽 하나만** 점등. 동시 점등 금지.
- Points 탭(성장)·무료 탭(군단)은 통화가 달라 독립 점등.
- **가격을 상수로 박지 않는다.** 런타임에 `EquipCosts[tier]`/`SigilCost`를 읽는다
  — entry 6(각인 12유물 미서명)이 움직여도 규칙이 스스로 답을 낸다.
- 각인 행 컨텍스트는 대상 스테이지의 **실효 해저드 표**(`HazardOverride ?? 앵커`)
  로 판정한다. 서약 협정은 제외 — 방문별 옵트인이라, 토글에 따라 바뀌는 라벨은
  플레이어가 아직 고르지 않은 런에 대한 조언이 된다.

### §20 폰트 계약 정정

`gen_hud_font.sh`의 따옴표 정규식 `"([^"\\]*)"` 는 백슬래시 포함 리터럴을 먹지
않아 짝짓기가 어긋나고 **문자열 통째로 매치 사이 틈에 빠진다**. 이 경로로
U+2212가 charset에 들어가지 않았고, 커버리지 검사는 같은 charset과 폰트를
비교하므로 초록이었다 — **자기 사각지대를 기준으로 검증하는 검사기**였다.

- 생성기: 이스케이프 인식 규칙을 union으로 추가.
- `FontCoverageTests`: 한글 전용 수집을 **따옴표 안 비ASCII 전수**로 확장.
  테스트가 생성기보다 좁으면 그 차집합은 영원히 사각이다.
- 소스 폰트 NanumBarunGothic에 **U+00B7·U+2212가 없다** — 재생성으로 닫을 수
  없는 부류다. 각각 `•`(U+2022)·ASCII `-`로 치환했다.
- 현행: **500 글리프, coverage FULL.**

### 검증

- 골든 7스위트 무이동 (순수 뷰이므로 필수 조건).
- 지목 1024 상태 전수: 슬롯 수 ∈ {0,1}, `None`은 전 클리어+숙달에서만.
- 잠금 사유: 빈 문자열 0, 사유 2종 분류 100%, 선행명 포함.
- 배지: 유물 탭 동시 점등 0건, 오도 ≤ 2유물.
- 터치 플로어: 4상태 합집합이 동결표와 일치.
- 폰트: `coverage: FULL` + 글리프 수 변동 시 otf 재생성 커밋 동반.

---

## AMENDMENT #9 — 인게임 안내 (run-id 20260807-ingame-guidance)

**심 수치 변경 0. 저장 필드 1개 추가(int). 순수 뷰 + 세이브 확장.**

#8과 성격이 같다: 이미 있는 것을 화면에 옮긴다. 다른 점은 "무엇을 봤는가"를
기억해야 하므로 세이브가 한 칸 늘어난다는 것뿐이다. **골든 7스위트 무이동이
전제**이며, 정지는 timeScale만 건드리므로 틱 크기와 입력 규칙이 불변이다.

### §21 안내 카탈로그 (23종)

`GuidanceCatalog.Entries` — 비트 인덱스 = 배열 인덱스. **영구 고정.**
추가는 안전, **재정렬은 조용히 전부를 다시 가르친다.**
...
문안의 모든 수치는 **상수를 보간**한다. 재입력 금지 — 밸런스가 움직이면
레슨이 따라 움직여야 하고, 심에서 어긋난 레슨은 없느니만 못하다.

### §22 정지 예산

| 항목 | 값 |
|---|---|
| 정지 총 대상 | **정확히 8** (기믹 6 + 승패 2) |
| 한 스테이지 정지 상한 | **≤ 4** |
| 카드 1장 어절 상한 | **33** |
| 재정지 | **없음** — 23비트가 항목 단위 1회를 보장 |
| 해제 | **아무 키·탭·클릭·패드 버튼** |

8은 장르 대역(중앙값 0, 확인 최댓값 1) **밖이다**. 의도적 이탈이며 근거는
서베이의 핵심 발견이다: Returnal은 무정지인데 과잉으로 혹평받았고 Into the
Breach는 정지를 쓰는데 불만이 0 — **변수는 분량이 아니라 거절 가능성이다.**

### §23 트리거

| 대상 | 트리거 |
|---|---|
| 기믹 | 스테이지 시작 시 **실효 해저드 표 1회 스캔** |
| 승리 | `BossSpawned` |
| 패배 | **첫 `PlayerDamaged`** — `GameOver`면 `ResetRunUi`가 카드를 뜯어 읽을 수 없다 |
| 조작 | 기름이 그 비용에 도달한 첫 프레임 |
| 아이템 | 첫 획득 |
| 돌발 | `PerilOpened` / `SurgeOpened` |

**시련 제외.** 큐는 **한 번에 한 장**만 뽑는다 — 두 장이 겹치면 한 번의
키 입력으로 둘 다 닫힌다.

### §24 저장 (v6)

`CampaignData.GuidanceSeen`, int 1개, 23비트 사용 / 32 상한.
additive: pre-v6 세이브는 키가 없어 0으로 로드되고 그것이 정확한 사실이다.
...
전량 몰수는 장르 소수파(2/16)다. 근거는 국지적이다 — **패배도 유물을
적립하므로**(`GameDirector.cs:614-626`) 등급 몰수를 주면 "줍고 나가기"가
죽을 위험 없는 최적 파밍이 된다.

### §26 timeScale 0 계약

- 정지 중 `Time.timeScale = 0`. `ApplyTimeScale` **최상단에서 조기 반환** —
  스무딩 이즈백이 카드 아래에서 1로 걸어 올라가면 안 된다.
- 해제 폴링은 `MonoBehaviour.Update`(무스케일 벽시계). `Time.deltaTime == 0`
  이라 스케일 기반 타이머는 영원히 안 돈다.
- `ResetRunUi`가 **모든 종료 경로에서** 카드를 뜯는다. 콜백은 발화하지
  않는다 — 못 읽은 레슨을 소비하면 안 된다.
- 이탈 모달도 정지에 포함된다. 결정하는 동안 죽으면 신중함을 처벌한다.

콘솔이 같은 함정을 0.2로 이미 밟았다(`HudView:904-907` 가드).
0은 정도의 차이가 아니라 종류의 차이 — 하드 프리즈다.
...
이 사이클이 닫은 겹침 4건(A 14,960u² / B1 27,568u² / D 9,000u² /
D2 5,200u²)이 전부 이 유형이다. 정지 카드와 이탈 모달은 본체·라벨·버튼을
한 함수에서 만든다.

### 검증

- 골든 7스위트 무이동 (순수 뷰).
- 카탈로그: 비트 == 인덱스 전수, 정지 계층 정확히 8, 어절 ≤33.
- 트리거: `HazardKind`/`PickupKind` 열거 전수가 유효 비트로 매핑.
- 저장: `GuidanceSeen` 라운드트립 + pre-v6 blob이 0으로 로드.
- 정지: 카드 중 체력 불변, 종료 경로에서 timeScale 복원.
- 이탈: 유물 증가 0, `ClearedMask` 불변.
- rect: 사망 패널·정지 카드·이탈 모달 겹침 0.
- 폰트: 카탈로그 문자열 전수 `HasCharacter`. 현행 **532 글리프 FULL**.
