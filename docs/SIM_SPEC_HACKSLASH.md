...
<!-- 이 세 행은 첫 등록 때 16·17·18이었다. 같은 PR이 리뷰되는 동안 main이
     ...
     (#8/#9/#10)은 그대로다 — 부록 A 머리의 규칙 그대로, 이 표만 움직인다.
     로컬 절 번호도 그대로다: #17의 §16–§20은 자기 증보 안에서만 의미를 갖고
     Amendment #16의 §20과 겹치는 것은 이 원장이 이미 여러 번 기록한 형태다. -->
...
#8(동료 시그니처)·#9(모멘텀)·#10(훈련장·돌발)을 이미 소비했다. 부록 A 머리의
규칙을 그대로 적용한다 — **기존 헤딩 문자열은 재작성하지 않는다.** 코드 주석·
커밋 메시지·QA 리포트가 도착 헤딩을 인용하고 있고, 문자열을 밀면 그 인용이
전부 끊긴다. 원장이 유일한 진실이다:

| 도착 헤딩 | canonical | 로컬 절 번호 |
|---|---|---|
| `AMENDMENT #8 — 진행 네비게이션` | **17** | §16–§20 |
| `AMENDMENT #9 — 인게임 안내` | **18** | §21–§27 |
| `AMENDMENT #10 — 안내 탭 가독성` | **19** | 없음 (순수 뷰) |

**이 표는 한 번 밀렸다.** 첫 등록은 16·17·18이었는데, 같은 PR이 리뷰되는 동안
main이 Amendment #16(보스 아키타입)을 올렸다. 도착 헤딩은 그대로고 원장 행만
움직였다 — 정확히 이 장치가 존재하는 이유다.

로컬 절 번호가 겹치는 것은 새 사고가 아니다: 부록 A가 Amendment #11에 대해
"로컬 절 번호 §16"을, #16에 대해 "§20"을 이미 그렇게 기록하고 있고, 로컬
번호는 자기 증보 안에서만 의미를 갖는다.
